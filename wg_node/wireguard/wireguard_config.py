...
__all__ = ("WIREGUARD_CONFIG", "generate_peer_address")

KEY_STORAGE_PATH = "/etc/wg-node/server-keys.json"
WG_CONFIG_PATH = "/etc/wireguard/wg0.conf"

WG_BASE_ADDRESS = "10.0.0.x"

SERVER_INTERFACE_ADDRESS = WG_BASE_ADDRESS.replace("x", "1")
SERVER_INTERFACE_LISTEN_PORT = 51820
...
def generate_peer_address(taken_addresses: list[str]) -> str | None:
    ...
    taken_addresses.append(SERVER_INTERFACE_ADDRESS)

    for x in range(2, 255):
        address = WG_BASE_ADDRESS.replace("x", str(x))
...
class WireguardConfig:
    ...
    def generate_config_content(self, peers: list[WireguardPeer]) -> str:
...
Address = {SERVER_INTERFACE_ADDRESS}/24
ListenPort = {SERVER_INTERFACE_LISTEN_PORT}

PreUp =
PostUp = iptables -t nat -A POSTROUTING -s {WG_BASE_ADDRESS.replace("x", "0")}/24 -o eth0 -j MASQUERADE; iptables -A INPUT -p udp -m udp --dport 51820 -j ACCEPT; iptables -A FORWARD -i wg0 -j ACCEPT; iptables -A FORWARD -o wg0 -j ACCEPT;
PreDown =
PostDown =
"""
